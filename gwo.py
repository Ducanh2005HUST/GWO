...
def plot_gwo_vector(alpha, beta, delta, wolf, iteration):
    """Vẽ sơ đồ minh hoạ hướng di chuyển của 1 con sói."""
    plt.figure(figsize=(5,5))
    points = np.array([alpha, beta, delta, wolf])

    plt.scatter(points[:,0], points[:,1], s=80, label='Points')

    # Vẽ các vector hướng
    plt.arrow(wolf[0], wolf[1], alpha[0]-wolf[0], alpha[1]-wolf[1],
              length_includes_head=True, head_width=0.1, label='-> alpha')
    plt.arrow(wolf[0], wolf[1], beta[0]-wolf[0], beta[1]-wolf[1],
              length_includes_head=True, head_width=0.1)
    plt.arrow(wolf[0], wolf[1], delta[0]-wolf[0], delta[1]-wolf[1],
              length_includes_head=True, head_width=0.1)

    plt.title(f"GWO Movement at Iter {iteration}")
    plt.grid(True)
    plt.xlabel("x")
    plt.ylabel("y")
    plt.show()

def GWO(fitness, num_wolves=20, dim=2, max_iter=50, lb=-10, ub=10, visualize=False):
    ...
    for t in range(max_iter):
        ...
        for i in range(num_wolves):
            ...
            if visualize and i == 0 and dim == 2:
                plot_gwo_vector(alpha, beta, delta, wolves[i], t)
